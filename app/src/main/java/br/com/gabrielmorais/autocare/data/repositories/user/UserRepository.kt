...
class UserRepository(private val userDao: UserDao) : IUserRepository {
  ...
  override suspend fun createUser(user: User) = try {
    userDao.create(user)
  } catch (e: Exception) {
    throw e
  }

  override fun create(user: User): Flow<Resource<User?>> {
    return flow {
  ...
  override fun getById(userId: String): User = try {
    userDao.getById(userId)
  } catch (e: Exception) {
    throw e
  }

  override suspend fun getByNickName(nickname: String): User = try {
    userDao.getUserByNickname(nickname)
  } catch (e: Exception) {
    throw e
  }

  override suspend fun update(user: User) : Int = try {
    userDao.update(user)
  } catch (e: Exception) {
    throw e
  }

  override suspend fun delete(user: User) = try {
    userDao.delete(user)
  } catch (e: Exception) {
    throw e
  }

  override fun getVehicles(userId: String): Flow<List<Vehicle>> = try {
